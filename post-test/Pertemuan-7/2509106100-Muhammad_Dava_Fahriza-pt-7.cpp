...
void tambahProduk(Skincare *lemari, int *jumlah) {
    try {
        if (*jumlah >= 100)
            throw runtime_error("Kapasitas penuh! Tidak bisa menambah produk.");
...
void tambahStok(Skincare *lemari, int jumlah) {
    try {
        ...
        if (nomor <= 0 || nomor > jumlah)
            throw out_of_range("Nomor produk tidak valid.");

        cout << "Tambah stok: ";
        cin >> tambah;

        if (tambah <= 0)
            throw invalid_argument("Jumlah tambah stok harus lebih dari 0.");
...
void kurangiStok(Skincare *lemari, int jumlah) {
    try {
        ...
        if (kurang <= 0)
            throw invalid_argument("Jumlah kurang stok harus lebih dari 0.");

        if (kurang > (lemari + nomor - 1)->stok)
            throw runtime_error("Stok tidak cukup untuk dikurangi.");
...
void linearSearchMerk(Skincare *lemari, int jumlah, string cari) {
    try {
        ...
        throw runtime_error("Produk '" + cari + "' tidak ditemukan.");
    } catch (const runtime_error &e) {
        cout << "[Error] " << e.what() << endl;
    }
}
    ...
    do {
        ...
        if (pilihan == 1) tambahProduk(lemari, &jumlah);
            ...
            try {
                ...
                cin >> cari;
                if (cari.empty()) throw invalid_argument("Input merk tidak boleh kosong.");
            ...
            try {
                ...
                if (cari < 0) throw invalid_argument("Stok yang dicari tidak boleh negatif.");
                ...
                if (hasil != -1)
                    ...
                    throw runtime_error("Stok " + to_string(cari) + " tidak ditemukan di data manapun.");
            } catch (const invalid_argument &e) {
                cout << "[Input Tidak Valid] " << e.what() << endl;
            } catch (const runtime_error &e) {
                cout << "[Error] " << e.what() << endl;
